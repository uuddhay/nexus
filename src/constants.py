...
DATA_DIR = os.getenv("NEXUS_DATA_DIR", get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place NEXUS_DATA_DIR is read. Import these constants instead of
...
MAIL_ATTACHMENTS_DIR = os.getenv("NEXUS_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to Nexus's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. NEXUS_INTERNAL_BASE  - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("NEXUS_INTERNAL_BASE")
    if override:
