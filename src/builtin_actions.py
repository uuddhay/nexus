...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("NEXUS_SCRIPT_HOST", "localhost")).strip()
...
async def action_check_email_urgency(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip Nexus-generated reminders so the scanner
                            ...
                            _ody_origin = (msg.get("X-Nexus-Origin") or "").strip().lower()
                            _ody_kind = (msg.get("X-Nexus-Kind") or "").strip().lower()
                            ...
                            if (_ody_origin == "nexus-ui" or _ody_kind == "reminder"
                                    or _raw_subj.startswith("reminder (nexus):")
        ...
        if new_urgent:
            ...
            # straight into the Nexus Email tab).
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on NEXUS_SCRIPT_HOST",
