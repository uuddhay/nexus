...
Nexus keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/nexus-backup` tool
...
```bash
# Create a snapshot → backups/nexus-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/nexus-backup snapshot

# List existing snapshots (most recent first)
./scripts/nexus-backup list

# Check a tarball's integrity without extracting it
./scripts/nexus-backup verify backups/nexus-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/nexus-backup restore backups/nexus-backup-20260101-120000.tar.gz --yes
```
...
./scripts/nexus-backup snapshot --out /mnt/nas/nexus-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/nexus-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/nexus && ./scripts/nexus-backup snapshot --out "/mnt/nas/nexus-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `nexus-backup` therefore does not capture the Docker
