...
<#
  Build a portable Windows distribution for Nexus.

  Output layout:
    dist\Nexus\Nexus.exe
    dist\Nexus\static\...
    dist\Nexus\scripts\...
    dist\Nexus\mcp_servers\...
    dist\Nexus\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Nexus @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Nexus" -ForegroundColor Green
