                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Nexus (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Nexus" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "nexus_kind" if "nexus_kind" in cols else "'scheduled' AS nexus_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `nexus-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the NEXUS_INPROCESS_POLLERS=0 guidance) -
                ...
                nexus_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Nexus-Origin"] = "nexus-ui"
                outer["X-Nexus-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", nexus_kind or "scheduled")[:64]
                outer["X-Nexus-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `nexus-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `NEXUS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `nexus-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("NEXUS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `NEXUS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (NEXUS_INPROCESS_POLLERS=0); "
            "drive `nexus-mail poll-scheduled` externally."
        )
        return
    import asyncio
