...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. Nexus "
...
# Override at deploy time via NEXUS_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "nexus_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN nexus_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("NEXUS_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for Nexus-generated mail (e.g. reminder, scheduled).
    nexus_kind: Optional[str] = None
