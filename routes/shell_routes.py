...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside Nexus."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside Nexus.",
        )
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "nexus-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f'NEXUS_USER_SHELL="${{SHELL:-}}"\n'
        f'if [ -n "$NEXUS_USER_SHELL" ] && [ -x "$NEXUS_USER_SHELL" ]; then\n'
        f'  NEXUS_USER_PATH="$("$NEXUS_USER_SHELL" -ic \'printf "__NEXUS_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__NEXUS_PATH__//p\' | tail -n 1 || true)"\n'
        f'  if [ -n "$NEXUS_USER_PATH" ]; then export PATH="$NEXUS_USER_PATH:$PATH"; fi\n'
        f"fi\n"
