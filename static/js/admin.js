...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('nexus:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('.adm-quickstart-section').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'nexus.addModels.' + sec.id + '.open';
    let open = false;
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { window.dispatchEvent(new CustomEvent('nexus-integrations-changed')); } catch (_) {}
      });
    });
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'nexus_backup.json';
