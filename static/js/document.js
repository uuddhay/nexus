...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'nexus.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__nexusActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'nexus-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'nexus-doc-minimized-' + sessionId;
  ...
  let _nexusAttachMenu = null;

  function _closeNexusAttachMenu() {
    if (_nexusAttachMenu) {
      _nexusAttachMenu.remove();
      _nexusAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_nexusAttachMenu && !_nexusAttachMenu.contains(e.target)) _closeNexusAttachMenu();
  }
  ...
  function _positionNexusAttachMenu(anchor, menu) {
  ...
  function _nexusAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageNexusAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-nexus`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageNexusZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-nexus-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterNexusAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachNexusItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageNexusAttachment(kind, id);
      if (!data) return;
      _afterNexusAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeNexusAttachMenu();
    } catch (err) {
      console.error('Failed to attach Nexus item:', err);
      if (uiModule) uiModule.showError('Failed to attach from Nexus');
    }
  }

  function _selectedNexusAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-nexus-attach-row.is-selected') || []);
  }

  function _syncNexusAttachSelection(menu) {
    const selected = _selectedNexusAttachRows(menu);
    const bar = menu?.querySelector?.('.email-nexus-attach-actions');
    const count = menu?.querySelector?.('.email-nexus-attach-count');
    const attachBtn = menu?.querySelector?.('.email-nexus-attach-selected');
  ...
  async function _attachSelectedNexusItems(menu) {
    const rows = _selectedNexusAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-nexus-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageNexusZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageNexusAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterNexusAttachmentsAdded(added, zip ? 'nexus-attachments.zip' : undefined);
      _closeNexusAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected Nexus items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from Nexus');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadNexusAttachItems(menu, kind) {
    ...
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-nexus-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncNexusAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _nexusAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-nexus-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-nexus-attach-dot" aria-hidden="true"></span>
            <span class="email-nexus-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-nexus-attach-main">
              <span class="email-nexus-attach-title">${_escHtml(label)}</span>
              <span class="email-nexus-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-nexus-attach-dot" aria-hidden="true"></span>
            <span class="email-nexus-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-nexus-attach-main">
              <span class="email-nexus-attach-title">${_escHtml(label)}</span>
              <span class="email-nexus-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachNexusItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncNexusAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load Nexus attach items:', err);
      list.innerHTML = '<div class="email-nexus-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-nexus-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-nexus-attach-local">
      ...
      <label class="email-nexus-attach-search-wrap">
        ...
        <input type="search" class="email-nexus-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-nexus-attach-list"></div>
      <div class="email-nexus-attach-actions">
        <span class="email-nexus-attach-count"></span>
        <button type="button" class="email-nexus-attach-selected" disabled>
    ...
    _nexusAttachMenu = menu;
    _positionNexusAttachMenu(anchor, menu);
    menu.querySelector('.email-nexus-attach-local')?.addEventListener('click', () => {
      _closeNexusAttachMenu();
    ...
    _loadNexusAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__nexusActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'nexus:email-ai-reply-context:v1:';
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('nexus-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('nexus-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'nexus-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('nexus-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('nexus-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('nexus-suggestions-' + docId);
      if (!raw) return;
