// ============================================
// Nexus UI — Main Application Orchestrator
...
function _isForegroundChatBusy() {
  ...
  return !!window.__nexusChatBusy
    || Date.now() < (window.__nexusChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__nexusQueueStreamingSubmit = now;
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__nexusDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (_opener) window._nexusRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('nexus-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__nexusSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'nexus-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message Nexus...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__nexusSetChatMode === 'function') {
          window.__nexusSetChatMode('chat');
        } else {
        ...
        if (typeof window.__nexusSetChatMode === 'function') {
          window.__nexusSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'nexus-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'nexus-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('nexus:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__nexusQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
function startNexusApp() {
  if (window.__nexusAppStarted) return;
  window.__nexusAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__nexusChatBusyUntil = Math.max(window.__nexusChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('nexus-doc-open-' + _curSession) === '1') {
  ...
  window._nexusBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__nexusQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._nexusRouteOpener) {
          try { window._nexusRouteOpener(); } catch (_) {}
          window._nexusRouteOpener = null;
        }
      });
  } else {
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__nexusChatBusy
        || Date.now() < (window.__nexusChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
