"""cmd_add (scripts/nexus-memory) must tolerate a non-dict row in the
...
def _load_cli(monkeypatch):
    ...
    path = ROOT / "scripts" / "nexus-memory"
    loader = importlib.machinery.SourceFileLoader("nexus_memory_cli_add", str(path))
