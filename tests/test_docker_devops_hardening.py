...
def _compose_env_names(path: Path) -> set[str]:
    ...
    env = compose["services"]["nexus"]["environment"]
...
def _upload_limit_env_names() -> set[str]:
    ...
    return set(re.findall(r'"(NEXUS_[A-Z_]*BYTES)"', source)) | {
        "NEXUS_CHAT_UPLOAD_MAX_BYTES"
    }
...
def test_host_docker_overlay_mounts_socket_and_adds_docker_group():
    ...
    service = overlay["services"]["nexus"]
...
def test_docker_entrypoint_does_not_resolve_root_commands_from_app_local_path():
    ...
    setup_call = script.index('"$GOSU_BIN" "$NEXUS_USER" "$PYTHON_BIN" /app/setup.py')
    final_exec = script.index('exec "$GOSU_BIN" "$NEXUS_USER" "$@"')
