"""Regression: `nexus-calendar list` must select events that OVERLAP the
...
`nexus-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "nexus-calendar"
    loader = importlib.machinery.SourceFileLoader("nexus_calendar_cli", str(path))
